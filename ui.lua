...
frame.Position = UDim2.new(0.5, -300, 0.45, -200) -- Start higher  
...
Instance.new("UIPadding", sidebar).PaddingTop = UDim.new(0, 10)
		...
		for name, tabData in pairs(tabs) do
			...
			if tabButtons[name] then
				...
				if tabButtons[name]:IsA("TextButton") then
					tween(tabButtons[name], { TextColor3 = isVisible and BuilderUI.ActiveTheme.Text or BuilderUI.ActiveTheme.MutedText }):Play()
				end
			end
		end
	...
	function api:CreateTab(name, icon)
		...
		tabButton.MouseButton1Click:Connect(function() setActiveTab(name) end)
